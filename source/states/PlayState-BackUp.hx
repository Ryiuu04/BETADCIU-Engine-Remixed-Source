...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.x = FlxMath.lerp(iconP1.x, healthBar.x + (healthBar.width * healthPercent - iconOffset), 0.35/(ClientPrefs.data.framerate / 60));
		iconP2.x = FlxMath.lerp(iconP2.x, healthBar.x + (healthBar.width * healthPercent) - (iconP2.width - iconOffset), 0.35/(ClientPrefs.data.framerate / 60));
	...
	*/
	...
	function updateAccuracy(?badHit:Bool = false) 
	{
		...
		if (ClientPrefs.data.psychUI)
			scoreTxt.text = 'Score: ' + numberWithCommas(songScore) + ' | Misses: ' + songMisses + ' | Rating: ' + ratingName + ' (' + ratingPercent * 100 + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
		else
